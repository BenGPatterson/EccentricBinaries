...
def gen_grid_data(scaling_norms, e_vals, MA_vals, n_vals, fid_e, q, f_low, approximant='TEOBResumS'):
    ...
    # Calculate grid for each value of n
    for n in n_vals:
        start = time.time()
        zero_ecc_chirp = fid_e**(6/5)*scaling_norms[0]/(scaling_norms[1]**(6/5))
        all_matches[n] = chirp_match_MA_grid_data(e_vals, MA_vals, n, fid_e, zero_ecc_chirp, q, f_low, harm_ids[:n], approximant=approximant)
        end = time.time()
...
if __name__ == "__main__":
    ...
    gen_grid_data([10, 0.03], np.linspace(0, 0.5, 201), np.linspace(0, 2.5*np.pi, 40, endpoint=False), np.linspace(3, 8, 6).astype(int), 0.063, 2, 10, approximant='TEOBResumS')

...
def gen_grid_data(eff_chirp_vals, param_vals, MA_vals, n, fid_e, q, f_low, approximant='TEOBResumS'):

    all_matches = {}

    # Calculate grid for each chirp mass
    for eff_chirp in eff_chirp_vals:
        start = time.time()
        all_matches[eff_chirp] = chirp_match_MA_grid_data(param_vals, MA_vals, n, fid_e, eff_chirp, q, f_low, approximant=approximant)
        end = time.time()
        print(f'Effective chirp mass {eff_chirp}: Completed in {end-start} seconds.')
...
if __name__ == "__main__":
    ...
    gen_grid_data(np.linspace(20, 30, 5), np.linspace(0,0.5,201), np.linspace(0, 2*np.pi, 32, endpoint=False), 4, 0.1, 2, 10, approximant='TEOBResumS')

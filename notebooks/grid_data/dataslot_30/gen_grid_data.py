...
def single_match(s_f_e, e_chirp, wf_hjs, q, f_low, ovlps, ovlps_perp, harm_ids, psd, approximant='TEOBResumS'):
    ...
    for i in range(len(wf_hjs)):
        match_dict[i] = match_cplx[i]
    coeff_dict = estimate_coeffs(match_dict, ovlps, ovlps_perp)
    coeff_cplx = []
    for i in range(len(wf_hjs)):
        coeff_cplx.append(coeff_dict[i])
    ...
    # Repeat for coefficients
    coeff_quantities = []
    for i in range(1, len(wf_hjs)):
        coeff_quantities.append(np.abs(coeff_cplx[i])/np.abs(coeff_cplx[0])) # Single harmonic
        if i > 1:
            num = 0
            for j in range(1,i+1):
    ...
    return *match_cplx, np.linalg.norm(match_cplx), *match_quantities, *coeff_cplx, np.linalg.norm(coeff_cplx), *coeff_quantities

def chirp_match_MA_grid_data(param_vals, MA_vals, n, fid_e, zero_ecc_chirp, q, f_low, harm_ids, approximant='TEOBResumS'):
    ...
    all_wfs, ovlps, ovlps_perp = get_h([1]*n, f_low, fid_e, chirp2total(fid_chirp, q), q, sample_rate, approximant=approximant, return_ovlps=True)
    ...
    # Repeat for coefficients
    for i in range(n):
        matches[f'h{harm_ids[i]}_c'] = np.abs(match_arr[:,count+i].reshape(-1, len(MA_vals)))
        matches[f'h{harm_ids[i]}_phase_c'] = np.angle(match_arr[:,count+i].reshape(-1, len(MA_vals)))
    matches['quad_c'] = match_arr[:,count+n].reshape(-1, len(MA_vals))
    count += n+1
    for i in range(1,n):
        matches[f'h{harm_ids[i]}_h0_c'] = match_arr[:,count].reshape(-1, len(MA_vals))
        count += 1
        if i > 1:
            str_combo = ''
            for j in range(1, i+1):
                str_combo += f'h{harm_ids[j]}_'
            matches[f'{str_combo}h0_c'] = match_arr[:,count].reshape(-1, len(MA_vals))
            count += 1
            matches[f'{str_combo}h0_pc_c'] = match_arr[:,count].reshape(-1, len(MA_vals))
            count += 1
...
def gen_grid_data(scaling_norms, param_vals, MA_vals, n, fid_e_vals, q, f_low, max_e, approximant='TEOBResumS'):
    ...
    for fid_e in fid_e_vals:
        ...
        all_matches[zero_ecc_chirp] = chirp_match_MA_grid_data(e_vals, MA_vals, n, fid_e, zero_ecc_chirp, q, f_low, harm_ids, approximant=approximant)
        end = time.time()
...
if __name__ == "__main__":
    ...
    gen_grid_data([10, 0.035], np.linspace(0, 20/3, 201), np.linspace(0, 2*np.pi, 32, endpoint=False), 4, np.linspace(0.07, 0.07, 1), 2, 10, 0.5, approximant='TEOBResumS')

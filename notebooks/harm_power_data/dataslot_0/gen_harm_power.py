...
def single_match(anomaly, e, chirp, q, f_low, f_match, n_gen, harm_ids, psd, total_SNR, approximant='TEOBResumS'):
    ...
    # Adjust data to requested total SNR
    ...
    return *SNR_list, h1_hn1, h1_hn1_pc, anomaly

def grid_data(vary_param, param_vals, MA_vals, n, n_gen, e, chirp, q, f_low, f_match, harm_ids, total_SNR, approximant='TEOBResumS'):
    ...
    long_wf = gen_wf(s_f_long, s_e_long, chirp2total(long_chirp, long_q), long_q, sample_rate, approximant)
    ...
    partial_single_match = partial(single_match, f_low=f_low, f_match=f_match, n_gen=n_gen, harm_ids=harm_ids, psd=psd, total_SNR=total_SNR, approximant=approximant)
    ...
    matches['anomaly'] = match_arr[:,n+2].reshape(-1, len(MA_vals))
    ...
    matches['total_SNR'] = total_SNR

    return matches

def gen_grid_data(param_vals, base_vals,  MA_vals, n, n_gen, f_low, f_match, total_SNR, approximant='TEOBResumS'):
    ...
    start = time.time()
    all_matches['vary_e'] = grid_data('e', param_vals[0], MA_vals, n, n_gen, *base_vals, f_low, f_match, harm_ids, total_SNR, approximant=approximant)
    end = time.time()
    print(f'Varying eccentricity: Completed in {end-start} seconds.')

    # Calculate grid for varying chirp mass
    start = time.time()
    all_matches['vary_chirp'] = grid_data('chirp', param_vals[1], MA_vals, n, n_gen, *base_vals, f_low, f_match, harm_ids, total_SNR, approximant=approximant)
    end = time.time()
    print(f'Varying chirp mass: Completed in {end-start} seconds.')

    # Calculate grid for varying mass ratio
    start = time.time()
    all_matches['vary_q'] = grid_data('q', param_vals[2], MA_vals, n, n_gen, *base_vals, f_low, f_match, harm_ids, total_SNR, approximant=approximant)
    end = time.time()
...
if __name__ == "__main__":
    ...
    gen_grid_data([np.linspace(0, 0.4, 6), np.linspace(10, 40, 6), np.linspace(1, 5, 6)], [0.2, 24, 2], np.linspace(0, 2.5*np.pi, 10, endpoint=False), 4, 6, 10, 20, 20, approximant='TEOBResumS')

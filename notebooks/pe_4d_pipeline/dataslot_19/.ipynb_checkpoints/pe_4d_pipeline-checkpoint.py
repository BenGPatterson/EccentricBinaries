...
def find_ecc_peak(data, base_dict, fid_dict, f_low, psd, n_ecc_harms, two_ecc_harms=True):
    ...
    init_e10 = np.max([bounds[0][0], init_e10])
...
def pipeline(data, init_guess, t_bounds, mismatch, n_ecc_harms, n_ecc_gen, f_low, psd,
    ...
    start = time.time()
    fid_dict, initial_metric = find_fid_point(base_dict, mismatch, snr, 'TEOBResumS-Dali-Harms', f_low, psd)
    end = time.time()
    ...
    peak_info['initial_metric'] = initial_metric

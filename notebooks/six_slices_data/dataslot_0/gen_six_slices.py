...
def six_slices_grid_data(fid_e, fid_chirp, fid_q, fid_chi1, e_vals, chirp_vals, q_vals, chi1_vals, n, n_gen, f_low, approximant='TEOBResumS'):
    ...
    wf_hjs = {}
    long_wf = gen_wf(f_low, e_vals[0], chirp2total(chirp_vals[0], q_vals[-1]), q_vals[0], sample_rate, chi1=chi1_vals[-1], approximant=approximant)
    ...
    # Loop over slices
    all_matches = {}
    dict_lbls = ['e', 'chirp', 'q', 'chi1']
    param_vals = [e_vals, chirp_vals, q_vals, chi1_vals]
    fid_params = [fid_e, fid_chirp, fid_q, fid_chi1]
    for slice_ids in list(combinations([0,1,2,3], 2)):
        start = time.time()
        fid_ids = [x for x in [0,1,2,3] if x not in slice_ids]

        # Generate list of all grid points
        param_dict = {}
        p1_vals, p2_vals = np.meshgrid(param_vals[slice_ids[0]], param_vals[slice_ids[1]], indexing='ij')
        param_dict[dict_lbls[slice_ids[0]]], param_dict[dict_lbls[slice_ids[1]]] = p1_vals.flatten(), p2_vals.flatten()
        grid_len = len(p1_vals.flatten())
        param_dict[dict_lbls[fid_ids[0]]], param_dict[dict_lbls[fid_ids[1]]] = np.full(grid_len, fid_params[fid_ids[0]]), np.full(grid_len, fid_params[fid_ids[1]])
        param_list = []
        for trial_e, trial_chirp, trial_q, trial_chi1 in zip(*[param_dict[key] for key in dict_lbls]):
        ...
        p1_len, p2_len = len(param_vals[slice_ids[0]]), len(param_vals[slice_ids[1]])
        all_matches[dict_lbls[slice_ids[0]]+','+dict_lbls[slice_ids[1]]] = save_slice_matches(match_arr, p1_len, p2_len, harm_ids)
        end = time.time()
        print(f'{dict_lbls[slice_ids[0]]}, {dict_lbls[slice_ids[1]]} slice finished in {end-start} seconds')
    ...
    all_matches['input_params']['chirp_vals'] = chirp_vals
    all_matches['input_params']['q_vals'] = q_vals
    all_matches['input_params']['chi1_vals'] = chi1_vals
...
if __name__ == "__main__":
    ...
    six_slices_grid_data(0.1, 24, 2, 0, np.linspace(0, 0.4, 31), np.linspace(22, 25, 31), np.linspace(1, 4, 31), np.linspace(0, 0.7, 31), 4, 20, 10, approximant='TEOBResumS')

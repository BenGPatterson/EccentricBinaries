...
def match_harms(wf_h, wf_t, f_low, psd, dominant_mode=0):
...
def single_match(e_chirp, wf_hjs_list, q, f_low, harm_ids, psd, approximant='TEOBResumS'):
    ...
    e, chirp = e_chirp
    s = {}
    all_wfs = list(get_h([1]*len(harm_ids), f_low, e, chirp2total(chirp, q), q, sample_rate, approximant=approximant))
...
if __name__ == "__main__":
    ...
    gen_e_sqrd_chirp_data([0, 0.05, 0.1, 0.15, 0.2], [10, 24, 50], np.sqrt(np.linspace(0, 0.5**2, 301)), [np.linspace(9.1, 10.1, 101), np.linspace(21,25,101), np.linspace(44,56,101)], 4, 2, 10, approximant='TEOBResumS')

...
def single_match(params, wf_hjs, f_gen, target_ang, f_low, psd, tlen, sample_rate, target_tol=0.03, max_iter=10):
    ...
    # Iterate to target MA
    tol = 2*np.pi
    best_tol = 2*np.pi
    current_iter = 0
    MA = 0
    s_factor = 1
    while tol > target_tol and current_iter < max_iter:

        # Shift to f, e for required MA (using approx eqns w/o correction for now
        s_f_2pi =  (10 - shifted_f(f_gen, e, M, q))*s_factor
        s_f = 10 - s_f_2pi*MA/(2*np.pi)
        s_e = shifted_e(s_f, f_gen, e)

        # Generate and match wf at requested MA
        hp, hc = generate_eccentric_waveform(M, q, s_e,  chi_eff, chi_eff, s_f, sample_rate, tlen=tlen, to_fs=False)
        h = hp - 1j*hc
        match_cplx = match_hn(wf_hjs, h, f_gen, psd=psd, f_match=f_low)
        phi_0 = np.angle(match_cplx[0])%(2*np.pi)
        phi_1 = np.angle(match_cplx[1])%(2*np.pi)
        meas_ang = (phi_1-phi_0)%(2*np.pi)

        # Update MA, tolerance, and iteration count
        MA += target_ang - meas_ang
        MA = MA%(2*np.pi)
        tol = np.abs(meas_ang-target_ang)
        current_iter += 1

        # Save if best tol so far
        if tol < best_tol:
            best_tol = tol
            best_match_cplx = match_cplx
            best_meas_ang = meas_ang

        # Calculate s_factor for future iterations
        if current_iter == 1:
            s_f_max = shifted_f(f_gen, e, M, q)
            s_e_max = shifted_e(s_f_max, f_gen, e)
            s_hp, s_hc = generate_eccentric_waveform(M, q, s_e_max, chi_eff, chi_eff, s_f_max, sample_rate, tlen=tlen, to_fs=False)
            s_h = s_hp - 1j*s_hc
    ...
    for i in range(1, len(wf_hjs)):
        match_quantities.append(np.abs(best_match_cplx[i])/np.abs(best_match_cplx[0])) # Single harmonic
        if i > 1:
            num = 0
            for j in range(1,i+1):
                num += np.abs(best_match_cplx[j])**2
            match_quantities.append(np.sqrt(num/np.abs(best_match_cplx[0]**2))) # Naive multiple harmonics
            if i == 2:
                pc_frac = comb_harm_consistent(np.abs(best_match_cplx[:i+1]), np.angle(best_match_cplx[:i+1]), harms=[0,1,-1])
                match_quantities.append(pc_frac) # Phase consistent multiple harmonics

    return *best_match_cplx, np.linalg.norm(best_match_cplx), *match_quantities, best_meas_ang

def degen_line_grid_data(base_dict, fid_dict, e_vals, target_ang, n_ecc_harms, n_ecc_gen, f_low, sample_rate):
    ...
    for i in range(len(param_vals['ecc_gen'])):
        param_list.append({'total_mass': param_vals['total_mass'][i],
    ...
    partial_single_match = partial(single_match, wf_hjs=wf_hjs, f_gen=f_gen, target_ang=target_ang, f_low=f_low,
                                   psd=psd, tlen=tlen, sample_rate=sample_rate)
    multi_ctx = get_context('forkserver')
    preload = ['numpy', 'calcwf', 'interpolating_match', 'simple_pe.waveforms', 'simple_pe.param_est']
    ...
    for i in range(n_ecc_harms):
        matches[f'h{harm_ids[i]}'] = np.abs(match_arr[:,i])
        matches[f'h{harm_ids[i]}_phase'] = np.angle(match_arr[:,i])
    matches['quad'] = np.abs(match_arr[:,n_ecc_harms])
    count = n_ecc_harms+1
    for i in range(1,n_ecc_harms):
        matches[f'h{harm_ids[i]}_h0'] = np.abs(match_arr[:,count])
        count += 1
        if i > 1:
            str_combo = ''
            for j in range(1, i+1):
                str_combo += f'h{harm_ids[j]}_'
            matches[f'{str_combo}h0'] = np.abs(match_arr[:,count])
            count += 1
            if i == 2:
                matches[f'{str_combo}h0_pc'] = np.abs(match_arr[:,count])
                count += 1
    matches['meas_ang'] = match_arr[:,-1]
    ...
    matches['metadata']['target_ang'] = target_ang
...
def pipeline(base_dict, mismatch, e_vals, n_ecc_harms, n_ecc_gen, f_low, sample_rate=4096):
    ...
    # Generate grid data
    start = time.time()
    all_matches = degen_line_grid_data(base_dict, fid_dict, e_vals, 4, n_ecc_harms, n_ecc_gen, f_low, sample_rate)
    end = time.time()
...
if __name__ == "__main__":
    ...
    all_matches = pipeline(base_dict, 8.76*10**-5, np.linspace(0, 0.5, 151), 4, 6, 20)

...
def pipeline(data, init_guess, t_bounds, mismatch, e_vals, MA_vals, n_ecc_harms, n_ecc_gen, f_low, psd, ifos, sample_rate=4096):
    ...
    # Generate grid data
    start = time.time()
    all_matches = degen_line_grid_data(base_dict, fid_dict, e_vals, MA_vals, n_ecc_harms, n_ecc_gen, f_low, sample_rate, psd['H1'])
    end = time.time()
...
if __name__ == "__main__":
    ...
    all_matches = pipeline(data, init_guess, t_bounds, 8.76*10**-5, np.linspace(0, 0.5, 151), np.linspace(0, 2*np.pi, 32), 4, 6,
                           f_low, psd, ifos, sample_rate=sample_rate)
